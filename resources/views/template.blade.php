<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    
    <link rel="stylesheet" href="{{ asset('/css/app.css') }}">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    {{-- <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script> --}}

    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>PS System</title>
</head>

<body style="background-color:white">
  <?php
  use App\Brand;
  $brandheader = Brand::all();
?>

    <!-- Navbar -->
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand" href="/"><img src="{{ asset('upload/PS.png') }}" height="65px" width="140px"></a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
      
        <div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
          <ul class="navbar-nav">
            <li class="nav-item active">
              <a class="nav-link" href="/">Home <span class="sr-only">(current)</span></a>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown"
              aria-haspopup="true" aria-expanded="false">
              <strong>Brands</strong>
          </a>
          
          <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
            @foreach ($brandheader as $brh)
            <a class="dropdown-item" href="/userPhones/{{$brh->id}}">{{$brh->brand_name}}</a> 
            @endforeach  
          </div>
          
            </li>

            <a href="/login" class="btn btn-outline-success mr-3">Login</a>
            <a href="/regis" class="btn btn-outline-danger">Daftar</a>
         
        </div>
      </nav>

    @yield('content')

  
</body>

<footer id="footer" class="footer-1">
  <div class="main-footer widgets-dark typo-light">
  <div class="container">
  <div class="row">
    
  <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget subscribe no-box">
      <h5 class="widget-title">PS SYSTEM<span></span></h5>
      <h6>Sistem yang bertujuan untuk memudahkan pemilik melakukan pencatatan data  seperti stok barang, jenis barang, dll. Akan ada beberapa menu misalnya menu laporan isinya grafik grafik, 
        menu penjualan isinya list item barang, harga, stok dan ada tombol buat cetak struk, dll.</h6>
    </div>
  </div>
  
  <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget no-box">
    <h5 class="widget-title">Quick Links<span></span></h5>
      <ul class="thumbnail-widget">
        <li>
          <div class="thumb-content"><a href="/">Get Started</a></div>
        </li>
        <li>
          <div class="thumb-content"><a href="#brands">Brands</a></div>
        </li>
        <li>
          <div class="thumb-content"><a href="/userListArticle">Article</a></div>
        </li>
        <li>
          <div class="thumb-content"><a href="#footer">About</a></div>
        </li>
      </ul>
    </div>
  </div>
  
  <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget no-box ml-2">
      <h5 class="widget-title">Metode Pembayaran<span></span></h5>
      <img src="https://eraspace.com/pub/media/wysiwyg/footer/All_Payment_Method_2_November_2020_2_.png" alt="" width="300rem">
    
    </div>
  </div>
  
  <div class="col-xs-12 col-sm-6 col-md-3">
    <div class="widget no-box ml-5">
      <h5 class="widget-title">Contact Us<span></span></h5>
      
      <p><a href="mailto:psSystem20@gmail.com" title="glorythemes" style="color: white">psSystem20@gmail.com</a></p>
      <ul class="social-footer2">
        <li class=""><a title="youtube" target="_blank" href="https://www.youtube.com/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a></li>
        <li class=""><a href="https://www.facebook.com/purchase.sale.142" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a></li>
        <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a></li>
        <li class=""><a title="instagram" target="_blank" href="https://www.instagram.com/pssystem/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a></li>
      </ul>
      <a class="btn mt-5" href="/regis" style="color: white">Register Now</a>
    </div>
  </div>
  
  </div>
  </div>
  </div>
    
  <div class="footer-copyright">
    <div class="container">
        <div class="row">
          <div class="col-md-12 text-center">
            <p class="text-sm-center">Copyright PSSYSTEM.COM © 2021. All rights reserved.</p>
          </div>
        </div>
    </div>
  </div>
  </footer>
</html>
